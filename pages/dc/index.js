...
export default function Home() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axios.get(
        `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&vote_count.gte=500&with_companies=429&with_original_language=en`
      );
  ...
  return (
    <div>
      <Head>
        <title>DC Comic Collection/EMDB</title>
        <meta name="keywords" content="web dev" />
      </Head>

      <div className={collectionStyle.video_wrapper}>
        <img src="/dc.jpg" className={collectionStyle.image} />
      </div>
